...
enum AppTheme {
    ...
    static let consoleBackground = Color(red: 0.025, green: 0.030, blue: 0.045)
...
struct TnCheatsBackground: View {
        ...
        TimelineView(.animation(minimumInterval: 1.0 / 30.0)) { timeline in
            ...
            ZStack {
                ...
                GeometryReader { proxy in
                    ...
                    ZStack {
                        orb(x: w * 0.20 + sin(t * 0.38) * 50, y: h * 0.15 + cos(t * 0.31) * 35, size: max(w, h) * 0.62, opacity: 0.22)
                        orb(x: w * 0.84 + cos(t * 0.29) * 70, y: h * 0.42 + sin(t * 0.34) * 45, size: max(w, h) * 0.52, opacity: 0.15)
                        orb(x: w * 0.46 + sin(t * 0.25) * 55, y: h * 0.90 + cos(t * 0.27) * 30, size: max(w, h) * 0.58, opacity: 0.12)
                    }
                }
                LinearGradient(
                    colors: [Color.black.opacity(0.18), Color.clear, Color.black.opacity(0.46)],
                    startPoint: .topLeading,
                    endPoint: .bottomTrailing
                )
                .ignoresSafeArea()
    ...
    private func orb(x: CGFloat, y: CGFloat, size: CGFloat, opacity: Double) -> some View {
        Circle()
            .fill(
                RadialGradient(
                    colors: [AppTheme.accent.opacity(opacity), AppTheme.accentSoft.opacity(opacity * 0.52), .clear],
                    center: .center,
                    startRadius: 0,
                    endRadius: size * 0.5
                )
