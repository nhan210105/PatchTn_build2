...
private struct GamePatchView: View {
    ...
    var body: some View {
        ...
        .sheet(item: $passwordRequest) { request in
            PasswordSheet(title: request.title, password: $password) {
                let selected = request.patches
                passwordRequest = nil
                let supplied = password
                password = ""
                applyMany(selected, password: supplied)
            }
        }
    ...
    private func patchRow(_ patch: BundledPatch) -> some View {
        ...
        return HStack(spacing: 11) {
            ...
            VStack(alignment: .leading, spacing: 3) {
                ...
                Text(patch.passwordProtected ? "Patch bảo vệ" : "Patch tích hợp")
    ...
    private func requestApply(_ patch: BundledPatch) {
        if patch.passwordProtected {
            passwordRequest = PasswordRequest(title: "Mật khẩu patch", patches: [patch])
        } else {
            applyMany([patch], password: nil)
        }
    }
    ...
    private func toggleMaster(_ category: BundledPatch.PatchCategory, enabled: Bool, items: [BundledPatch]) {
        ...
        if enabled {
            ...
            if needed.contains(where: { $0.passwordProtected }) {
                passwordRequest = PasswordRequest(title: "Mật khẩu patch", patches: needed)
            } else {
                applyMany(needed, password: nil)
            }
        } else {
            restoreMany(items.filter { states[$0.id] == true })
        }
    ...
    private func applyMany(_ items: [BundledPatch], password: String?) {
        ...
        for patch in items {
            ...
            DispatchQueue.global(qos: .userInitiated).async {
                do {
                    let project = try BundledPatchStore.decode(patch, password: patch.passwordProtected ? password : nil)
    ...
    private struct PasswordRequest: Identifiable {
        let id = UUID()
        let title: String
        let patches: [BundledPatch]
    }
}
